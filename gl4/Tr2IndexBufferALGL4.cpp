	...
	Tr2RenderContextAL & renderContext )
{
	...
	{
		GL_FAIL( *data = glMapBufferRange( GL_ELEMENT_ARRAY_BUFFER, offset, size ? size : ( GetTotalSizeInBytes() - offset ), lockType == LOCK_READONLY ? GL_MAP_READ_BIT : GL_MAP_INVALIDATE_RANGE_BIT | GL_MAP_WRITE_BIT ) );
	}
	return S_OK;
}
